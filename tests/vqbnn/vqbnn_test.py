...
class TestVQBNN(unittest.TestCase):
    ...
    def test_op(self):
        ...
        vqbnn = VQBNN(op, och_x_1, och_x, och_y)
        ...
        for x_0 in x_0s:
            ...
            vqbnn_runtimes.append(time3 - time2)

        dnn_runtime = (np.mean(dnn_runtimes), np.std(dnn_runtimes))
        vqbnn_runtime = (np.mean(vqbnn_runtimes), np.std(vqbnn_runtimes))
        print("DNN Runtime: %.3f ± %.3f (ms)" % (dnn_runtime[0] * 10 ** 3, dnn_runtime[1] * 10 ** 3))
        print("VQ-BNN Runtime: %.3f ± %.3f (ms)" % (vqbnn_runtime[0] * 10 ** 3, vqbnn_runtime[1] * 10 ** 3))
